...
/* Private typedef -----------------------------------------------9999999999999999999+++++++++++++9+------------*/
/* USER CODE BEGIN PTD */
...
#define NS 250       // Number of samples in LUT
...
/*
 * DMA handle (configuration and state of specific DMA channel),
 * src Address(where is the data comes from),
 * dest = where the data comes from, #No of data items to be transferred
 * */
  ...
  __HAL_TIM_ENABLE_DMA(&htim2,TIM_DMA_CC1);//Data transfer being started on channel 1 of TIM2
...
void EXTI0_1_IRQHandler(void)
{
	// TODO: Debounce using HAL_GetTick()
		...
		if(currentTime-lastPressTime < 500)//Compares the current time to the last time button pressed
		...
		lastPressTime = HAL_GetTick(); //stores the time value of button being pressed for next interrupt


	// TODO: Disable DMA transfer and abort IT, then start DMA in IT mode with new LUT and re-enable transfer
	// HINT: Consider using C's "switch" function to handle LUT changes
	__HAL_TIM_DISABLE_DMA(&htim2, TIM_DMA_CC1);//Function to disable the direct memory access(DMA) requests for a channel 1 of TIM 2
		HAL_DMA_Abort_IT(&hdma_tim2_ch1);
		/*
		 * DMA (direct memory access), allows for efficient data transfer to memory without using
		 * the CPU
		 *
		 * */
		if(i==0)
		{
			...
			delay(3000);
			lcd_command(CLEAR);
			lcd_putstring("Sawtooth");//Printing to LCD
			HAL_DMA_Start_IT(&hdma_tim2_ch1, saw_LUT, DestAddress, NS);
			//start data transfer
			__HAL_TIM_ENABLE_DMA(&htim2, TIM_DMA_CC1);//Enable data transfer
		}
		else if(i==1)
			{
				i = i+1;
		...
		else if(i==2)
			{
				...
				lcd_putstring("Sine");
				HAL_DMA_Start_IT(&hdma_tim2_ch1, Sin_LUT , DestAddress, NS);
				__HAL_TIM_ENABLE_DMA(&htim2, TIM_DMA_CC1); //Start data transfer
			}
