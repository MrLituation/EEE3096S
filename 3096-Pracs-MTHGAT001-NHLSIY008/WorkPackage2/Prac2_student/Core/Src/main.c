...
//We declare and array of specified 8bit integers that will be edited and changed
static uint8_t patterns[] = {0b10101010, 0b01010101, 0b11001100, 0b00110011, 0b11110000, 0b00001111};
//a global variable used to track the index is used
uint16_t indexTracker = 0x0;
/* USER CODE END PV */
...
int main(void)
{
  ...
  //This begins the Tim16 clock timer start the TIM16 process in interrupt (IT) mode
  HAL_TIM_Base_Start_IT(&htim16);

  // TODO: Write all "patterns" to EEPROM using SPI
  uint16_t address;
  //The eeprom address starts at 0x0
  /*The for loop loops through 6 patterns and assigns
  the value to its respective address in eeprom*/
  for(address = 0x0; address < 0x6; address++)
  {
	  //assigns individual pattern address to eeprom position
	  write_to_address(address, patterns[address]);
  }
  ...
  bool delayTime = false ;//used to alternate between 1/0.5 second delay
  ...
  while (1)
  {
	...
	// TODO: Check button PA0; if pressed, change timer delay
	  /* USER CODE BEGIN 3 */
	  if((GPIOA->IDR & SW0)!=1)//checks if button is pressed
	  {
		  delayTime = !delayTime;
		  //alternates the boolean value for delay time

	  }

	  if(delayTime)// delayTime = true: 0.5 seconds
	  {
		  TIM16 -> ARR = 499; //assigns a delay of 0.5s to auto-reload register

		  //delay(500000); //0.5 second delay
	  }
	  else //delayTime = false: 1 second delay time;
	  {
		  TIM16 -> ARR = 999; //assigns a delay of 1s to auto-reload register
...
void TIM16_IRQHandler(void)
{
	...
	//Assigns the value read from the array to a specific led light pattern as a variable
	uint8_t LEDValue = read_from_address(indexTracker);

	/*Failsafe: Checks if pattern in array corresponds to the LED
	value stored and read from eeprom*/
	if(patterns[indexTracker] == LEDValue)
	{
		GPIOB->ODR= LEDValue;
		//Assigns the output data register to be the value of LEDValue
		//Writes value to LEDS

	}
	else
	{
		GPIOB->ODR= 0b00000001;
		//If failsafe detects error displays single light to indicate failure
	}
	indexTracker++; //Increases the index for the next cycle when the interrupt occurs

	//If cycled through all pattern values; resets and starts from first stored value
	if(indexTracker>=sizeof(patterns)){
		indexTracker = 0;//sets back to 0
	}
}
